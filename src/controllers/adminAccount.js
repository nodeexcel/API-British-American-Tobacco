...
export class UserController extends BaseAPIController {
  ...
  create = (req, res) => {
    AdminProvider.create(this._db.Admin, req.body)
            .then((admin) => {
              this._db.Admin.findOne({ where: { email: admin.email } })
                    .then((data) => {
                      if (data) {
    ...
    this._db.Admin.find({ where: { email: req.body.email } })
            .then((admin) => {
              if (!admin) {
                ...
                let login = AdminProvider.login(req.body, admin.salt)
                        ...
                        .then((admin) => {
                          if (admin) {
                            this._db.Admin.update({ last_accessed: new Date() }, { where: { id: admin.id } })
                                    .then((data) => {
                                      let token = jwt.sign({ token: admin.id }, 'secret_key', { expiresIn: 60 * 60 })
